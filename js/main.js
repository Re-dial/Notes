...
function Save(){
	...
	document.getElementById("Card_sb").innerHTML = document.getElementById("Card_set").innerHTML;//找个地方先存起来
	Card_sb();																			// 在删除功能栏
	
	var body = document.getElementById("Card_sb").innerHTML;							// 获取body内容块
	
	document.getElementById("Card_sb").innerHTML = "";									// 在把刚才的地方清空......（；´д｀）ゞ
...
/*请无视.......*/
function Card_sb(){																		// kin,清空数据
	var paras = document.getElementById('Card_sb').getElementsByClassName('Toolbars');
	for(i=paras.length;i>=0;i--){
		if (paras[i] != null)
			paras[i].parentNode.removeChild( paras[i]);									// 删除功能栏
	}
}
